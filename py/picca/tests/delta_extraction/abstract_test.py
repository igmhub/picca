            ...
            for hdu_index, _ in enumerate(orig_hdul):
                ...
                for key in orig_header:
                    ...
                    if not key in ["CHECKSUM", "DATASUM"]:
                        if orig_header[key] != new_header[key] and (isinstance(
                                orig_header[key], str) or not np.isclose(
                ...
                if orig_data is None:
                    ...
                    for col in orig_data.dtype.names:
                        ...
                        if np.all(orig_data[col] != new_data[col]
                                 ) and not np.allclose(orig_data[col],
                                                       new_data[col],
                                                       equal_nan=True,
                                                       rtol=rtol):
                            print(f"\nOriginal file: {orig_file}")
