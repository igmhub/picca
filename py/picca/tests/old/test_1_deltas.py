...
class TestDelta(AbstractTest):
    ...
    def produce_forests(self):
        ...
        ra = vac[1]["RA"][:] * np.pi / 180.0
        dec = vac[1]["DEC"][:] * np.pi / 180.0
        thid = vac[1]["THING_ID"][:]
        plate = vac[1]["PLATE"][:]
        mjd = vac[1]["MJD"][:]
        ...
        ### Get Healpy pixels
        pixs = healpy.ang2pix(nside, np.pi / 2.0 - dec, ra)
    ...
    def produce_forests_minisv(self):
        ...
        ra = vac[1]["TARGET_RA"][:] * np.pi / 180.0
        dec = vac[1]["TARGET_DEC"][:] * np.pi / 180.0
        thid = vac[1]["TARGETID"][:]
