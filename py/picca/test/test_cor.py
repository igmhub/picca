...
class TestCor(unittest.TestCase):
    ...
    def test_cor(self):
        ...
        self.picca_base = resource_filename(
            'picca', './').replace('py/picca/./', '')
    ...
    def produce_forests(self):
        ...
        for p in np.unique(pixs):
            ...
            ###
            p_thid = thid[(pixs == p)]
            p_fl = numpy.random.normal(
                loc=1., scale=1., size=(log_lambda.size, p_thid.size))
            p_iv = numpy.random.lognormal(
                mean=0.1, sigma=0.1, size=(log_lambda.size, p_thid.size))
            p_am = np.zeros((log_lambda.size, p_thid.size)).astype(int)
            p_am[numpy.random.random(
                size=(log_lambda.size, p_thid.size)) > 0.90] = 1
        ...
        for i, _ in enumerate(m):
            ...
            for k in ld_m:
                ...
                if not np.array_equal(d_m, d_b):
                    userprint(
                        "WARNING: {}: Header key is {}, arrays are not exactly equal, using allclose".format(nameRun, k))
                    diff = d_m-d_b
                    w = d_m != 0.
                    diff[w] = np.absolute(diff[w]/d_m[w])
                    allclose = np.allclose(d_m, d_b)
                    self.assertTrue(allclose, "{}: Header key is {}, maximum relative difference is {}".format(
                        nameRun, k, diff.max()))
    ...
    def compare_h5py(self, path1, path2, nameRun=""):
        ...
        def compare_attributes(atts1, atts2):
            self.assertEqual(len(atts1.keys()), len(
                atts2.keys()), "{}".format(nameRun))
            self.assertListEqual(sorted(atts1.keys()), sorted(
                atts2.keys()), "{}".format(nameRun))
            for item in atts1:
                nequal = True
                ...
                if nequal:
                    userprint(
                        "WARNING: {}: not exactly equal, using allclose for {}".format(nameRun, item))
        ...
        def compare_values(val1, val2):
            if not np.array_equal(val1, val2):
                userprint(
                    "WARNING: {}: not exactly equal, using allclose".format(nameRun))
        ...
        self.assertListEqual(sorted(m.keys()), sorted(
            b.keys()), "{}".format(nameRun))
        ...
        with open(path, 'r') as f:
            for l in f:
                l = l.replace('\n', '').replace(
                    '==', ' ').replace('>=', ' ').split()
                self.assertTrue(
                    len(l) == 2, "requirements.txt attribute is not valid: {}".format(str(l)))
                req[l[0]] = l[1]
        ...
        for req_lib, req_ver in req.items():
            try:
                ...
                if local_ver != req_ver:
                    userprint("WARNING: The local version of {}: {} is different from the required version: {}".format(
    ...
    def send_export_cross_covariance_cf_xcf(self):
        ...
        returncode = subprocess.call(cmd, shell=True)
        self.assertEqual(
            returncode, 0, "export_cross_covariance_cf_xcf did not finish")
