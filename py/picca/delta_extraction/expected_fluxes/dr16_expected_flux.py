    ...
    def compute_delta_stack(self, forests, stack_from_deltas=False):
        ...
        for forest in forests:
            ...
            if Forest.wave_solution == "log":
                ...
                #TODO: this is definitely wrong, we do not have log-lambda quantities at this point, there might be subtleties as well
                ...
                raise ExpectedFluxError("Forest.wave_solution must be either "
                                        "'log' or 'linear'")
            #the indices are to exclude lambda>lambda_max from the stack                                        
            rebin = np.bincount(bins[bins<num_bins], weights=delta[bins<num_bins] * weights[bins<num_bins])
            stack_delta[:len(rebin)] += rebin
            rebin = np.bincount(bins[bins<num_bins], weights=weights[bins<num_bins])
    ...
    def populate_los_ids(self, forests):
        ...
        for forest in forests:
            ...
            if Forest.wave_solution=='log':
                ...
                stack_delta = self.get_stack_delta(lambda_)
                var_lss = self.get_var_lss(lambda_)
                eta = self.get_eta(lambda_)
                fudge = self.get_fudge(lambda_)
