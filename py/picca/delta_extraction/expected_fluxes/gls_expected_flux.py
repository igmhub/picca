"""This module defines the class GlsExpectedFlux"""
import logging
    ...
    def __parse_config(self, config):
        ...
        if self.force_stack_delta_to_zero is None:
            raise ExpectedFluxError(
                "Missing argument 'force stack delta to zero' required by GlsExpectedFlux"
            )

        limit_eta_string = config.get("limit eta")
        if limit_eta_string is None:
            raise ExpectedFluxError(
                "Missing argument 'limit eta' required by GlsExpectedFlux")
        ...
        if limit_var_lss_string is None:
            raise ExpectedFluxError(
                "Missing argument 'limit var lss' required by GlsExpectedFlux")
        ...
        if self.min_num_qso_in_fit is None:
            raise ExpectedFluxError(
                "Missing argument 'min qso in fit' required by GlsExpectedFlux"
            )

        self.num_iterations = config.getint("num iterations")
        if self.num_iterations is None:
            raise ExpectedFluxError(
                "Missing argument 'num iterations' required by GlsExpectedFlux"
            )

        self.order = config.getint("order")
        if self.order is None:
            raise ExpectedFluxError(
                "Missing argument 'order' required by GlsExpectedFlux")

        self.num_iter_inner = config.getint("num_iter_inner", 3)

        self.use_constant_weight = config.getboolean("use constant weight")
        if self.use_constant_weight is None:
            raise ExpectedFluxError(
                "Missing argument 'use constant weight' required by GlsExpectedFlux"
            )
        ...
        if self.use_ivar_as_weight is None:
            raise ExpectedFluxError(
                "Missing argument 'use ivar as weight' required by GlsExpectedFlux"
            )
        if self.use_ivar_as_weight:
            self.logger.warning(
            ...
            if self.num_processors > 1:
                with context.Pool(processes=self.num_processors) as pool:
                    arguments = [(forest, self.get_mean_cont, self.get_eta,
                                  ...
                                  self.use_constant_weight, self.order, self.num_iter_inner)
