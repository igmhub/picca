...
from picca.delta_extraction.rejection_logs.rejection_log_from_image import (
    RejectionLogFromImage,
)
from picca.delta_extraction.rejection_logs.rejection_log_from_table import (
    RejectionLogFromTable,
)
...
def _save_deltas_one_healpix_image(out_dir, healpix, forests):
    ...
    """
    results = fitsio.FITS(
        f"{out_dir}/Delta/delta-{healpix}.fits.gz", "rw", clobber=True
    )
...
class Data:
    ...
    def __parse_config(self, config):
        ...
        if wave_solution == "log":
            ...
            if pixel_step is None:
                raise DataError(
                    "Missing argument 'delta lambda' required by "
        ...
        if self.analysis_type not in accepted_analysis_type:
            raise DataError(
                "Invalid argument 'analysis type' required by "
        ...
        if self.input_directory is None:
            raise DataError("Missing argument 'input directory' required by Data")
        ...
        if self.num_processors is None:
            raise DataError("Missing argument 'num processors' required by Data")
        ...
        if not (
            rejection_log_file.endswith(".fits")
            or rejection_log_file.endswith(".fits.gz")
        ):
            raise DataError(
        ...
        if self.save_format == "BinTableHDU":
            ...
            raise DataError(
                "Invalid argument 'save format' required by "
    ...
    def filter_forests(self):
        ...
        for index, forest in enumerate(self.forests):
            if np.sum(forest.ivar > 0) < self.min_num_pix:
                ...
                self.logger.progress(
                    f"Rejected forest with los_id {forest.los_id} " "due to finding nan"
                )
    ...
    def find_nside(self):
        ...
        self.logger.progress(
            f"nside = {nside} -- mean #obj per pixel = " f"{mean_num_obj}"
        )
    ...
    def save_deltas(self):
        ...
        if self.num_processors > 1:
            ...
            with context.Pool(processes=self.num_processors) as pool:
                accepted_forests = pool.starmap(_save_deltas_one_healpix, arguments)
        else:
            accepted_forests = []
