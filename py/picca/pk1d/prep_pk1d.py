...
def exp_diff(hdul, log_lambda):
    ...
    if num_exp_per_col % 2 == 1:
        ...
        alpha = np.sqrt(4.0 * num_even_exp *
...
def spectral_resolution(wdisp,
    ...
    if with_correction:
        ...
        fiberid = fiberid % 500
        if fiberid < 100:
            correction = (1.0 + (correction - 1) * 0.25 +
...
def spectral_resolution_desi(reso_matrix, log_lambda):
    ...
    """
    delta_log_lambda = (log_lambda[-1] -
                        log_lambda[0]) / float(len(log_lambda) - 1)
    ...
    avg_reso_in_km_per_s = rms_in_pixel * SPEED_LIGHT * delta_log_lambda * np.log(
        10.0)
