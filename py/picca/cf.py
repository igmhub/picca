        ...
        for delta in data[healpix]:
            ...
            if data2 is not None:
                delta.neighbours = [
                    ...
                    if ((other_delta.z[-1] + delta.z[-1]) / 2.0 >= z_cut_min and
                        (other_delta.z[-1] + delta.z[-1]) / 2.0 < z_cut_max)
                ]
            else:
                delta.neighbours = [
                    other_delta for other_delta in neighbours
                    if (delta.ra > other_delta.ra and
                        (other_delta.z[-1] + delta.z[-1]) / 2.0 >= z_cut_min and
                        (other_delta.z[-1] + delta.z[-1]) / 2.0 < z_cut_max)
                ]
...
def compute_dmat_forest_pairs(
    ...
    if remove_same_half_plate_close_pairs and same_half_plate:
        weights12[abs(r_par[w]) < (r_par_max - r_par_min) /
                  num_bins_r_par] = 0.0
    ...
    for healpix in healpixs:
        for delta1 in data[healpix]:
            ...
            for delta2 in np.array(delta1.neighbours)[w]:
                r_comov1 = delta1.r_comov
                dist_m1 = delta1.dist_m
                z1_abs1 = 10**delta1.log_lambda / constants.ABSORBER_IGM[
                    abs_igm1] - 1
                ...
                r_comov2 = delta2.r_comov
                dist_m2 = delta2.dist_m
                z2_abs2 = 10**delta2.log_lambda / constants.ABSORBER_IGM[
                    abs_igm2] - 1
                ...
                if ((not x_correlation) and
                    ...
                    rebin = np.bincount(
                        (model_bins[w] + num_model_bins_r_par *
...
def compute_xi_1d_cross(healpix):
    ...
    for delta1 in data[healpix]:
        ...
        for delta2 in neighbours:
            ...
            num_pairs1d[bins] += (weights1 * weights2[:, None] >
                                  0.0).astype(int)
...
def compute_wickT123_pairs(
    ...
    for ind2 in i2:  # first figure out how many elements there are
        for ind1 in i1:
            ...
            w[ind1, ind2] = ((r_par < r_par_max) & (r_trans < r_trans_max) &
                             (r_par >= r_par_min))
            if w[ind1, ind2] > 0:
                wsum += 1
    if wsum == 0:
    ...
    for index1, p1 in enumerate(bins_forest12):
        ...
        for index2, p2 in enumerate(bins_forest12):
            ...
            if selected_bin1_12 == selected_bin1_13:
                ...
                # TODO: work on the good formula
                prod = (weighted_xi_1d_1[selected_bin1_12, selected_bin1_13] *
                        select_xi23)
                t5[p1, p2] += prod
                t5[p2, p1] += prod
