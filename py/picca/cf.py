...
def fill_neighs_x_correlation(pix):
    for ipix in pix:
        for d1 in data[ipix]:
            ...
            w = (ang<angmax)
            neighs = sp.array(neighs)[w]
            d1.neighs = [d for d in neighs if d1.thid != d.thid and (10**(d.ll[-1]- sp.log10(lambda_abs)) + 10**(d1.ll[-1] - sp.log10(lambda_abs)))/2. >= z_cut_min+1 and (10**(d.ll[-1]- sp.log10(lambda_abs)) + 10**(d1.ll[-1] - sp.log10(lambda_abs)))/2. < z_cut_max+1 ]
