...
from picca.pk1d.compute_pk1d import (
    Pk1D,
...
def process_all_files(index_file_args):
    ...
    if file_index % 5 == 0:
        userprint(
            "\rread {} of {} {}".format(file_index, args.len_files, num_data), end=""
        )
    ...
    if linear_binning:
        ...
        if (
            args.disable_reso_matrix
            or not hasattr(delta, "resolution_matrix")
    ...
    for delta in deltas:
        ...
        if args.parts_in_redshift:
            ...
            split_array = split_forest_in_z_parts(
                z_grid,
                delta.log_lambda,
                delta.delta,
                ...
                reso_matrix=(
                    delta.resolution_matrix if reso_correction == "matrix" else None
                ),
            ...
            first_pixel_index = (
                np.argmax(selected_pixels)
                if np.any(selected_pixels)
                else len(selected_pixels)
            )
            ...
            max_num_parts = (
                len(delta.log_lambda) - first_pixel_index
            ) // min_num_pixels
        ...
        for part_index in range(num_parts):
            ...
            if (
                ...
                if linear_binning:
                    exposures_diff_array[part_index] = rebin_diff_noise(
                        pixel_step,
                        lambda_array[part_index],
                        exposures_diff_array[part_index],
                    )
                else:
                    exposures_diff_array[part_index] = rebin_diff_noise(
                        pixel_step,
                        log_lambda_array[part_index],
                        exposures_diff_array[part_index],
                    )
            ...
            if args.nb_pixel_masked_max is not None:
                ...
                max_num_masked_pixels = int(
                    args.nb_pixel_masked_frac_max
                    * (np.max(lambda_new) - np.min(lambda_new))
                    / pixel_step
                )  # this only accounts for masking inside the spectrum, not at the adges
            else:
                max_num_masked_pixels = int(
                    args.nb_pixel_masked_frac_max
                    * (np.max(log_lambda_new) - np.min(log_lambda_new))
                    / pixel_step
                )
            ...
            k, fft_delta, pk_raw = compute_pk_raw(
                pixel_step, delta_new, linear_binning=linear_binning
            )

            # Compute pk_noise
            if (args.noise_estimate == "pipeline") | (
                args.noise_estimate == "mean_pipeline"
            ):
            ...
            if not running_on_raw_transmission:
                if linear_binning:
                    ...
                    delta_pixel = (
                        pixel_step * np.log(10.0) * constants.speed_light / 1000.0
                    )
                    correction_reso = compute_correction_reso(
                ...
                correction_reso = compute_correction_reso(
                    delta_pixel=pixel_step, mean_reso=0.0, k=k
                )
            ...
            if args.noise_estimate == "pipeline" or running_on_raw_transmission:
                ...
                if args.kmin_noise_avg is None and linear_binning:
                    ...
                    selection = (
                        (k > args.kmin_noise_avg)
                        if args.kmax_noise_avg is not None
                        else 1
                    ) & (
            ...
            if args.out_format == "fits":
                ...
                if file_out is None:
                    file_out = fitsio.FITS(
                        (args.out_dir + "/Pk1D-" + str(file_number) + ".fits.gz"),
                        "rw",
                        clobber=True,
                    )
...
def main(cmdargs=None):
    ...
    parser.add_argument(
        ...
        required=False,
        help=(
            "do not use the resolution matrix even "
            "if it exists and we are on linear binning"
        ),
    )

    parser.add_argument(
        "--add-pixelization-correction",
        default=False,
        action="store_true",
        required=False,
        help=(
            "Add a pixelization correction, as if the resolution  "
    ...
    if args.in_format == "fits":
        files = sorted(
            glob.glob(args.in_dir + "/*.fits.gz") + glob.glob(args.in_dir + "/*.fits")
        )
    elif args.in_format == "ascii":
    ...
    if args.nb_pixel_masked_frac_max is None and args.nb_pixel_masked_max is None:
        ...
        args.nb_pixel_masked_frac_max is None or args.nb_pixel_masked_max is None
    ):
        print(
            "both nb_pixel_masked_frac_max and nb_pixel_masked_max were set, using the latter"
        )
