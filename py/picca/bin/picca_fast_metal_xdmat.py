...
from picca import constants, io, xcf
...
def calc_fast_metal_dmat(
    in_lambda_abs, out_lambda_abs, stack_table, z_qso, weight_qso, rebin_factor=None
):
    ...
    if rebin_factor is not None:
        size = loglam.size
        loglam = (
            loglam[: (size // rebin_factor) * rebin_factor]
            .reshape((size // rebin_factor), rebin_factor)
            .mean(-1)
        )
        weight_forest = (
            weight_forest[: (size // rebin_factor) * rebin_factor]
            .reshape((size // rebin_factor), rebin_factor)
            .mean(-1)
        )
    ...
    # all pairs
    input_rp = (
        input_rf[:, None] - r_qso[None, :]
    ).ravel()  # same sign as line 528 of xcf.py (forest-qso)
    input_dist = ((input_rf[:, None] + r_qso[None, :]) / 2).ravel()

    # output
    output_zf = (10**loglam) / constants.ABSORBER_IGM[out_lambda_abs] - 1.0
    output_rf = xcf.cosmo.get_r_comov(output_zf)

    # all pairs
    output_rp = (
        output_rf[:, None] - r_qso[None, :]
    ).ravel()  # same sign as line 528 of xcf.py (forest-qso)
    ...
    weights = (
        (weight_forest * ((1 + input_zf) ** (alpha_in + alpha_out - 2)))[:, None]
        * weight_qso[None, :]
    ).ravel()

    # r_par distortion matrix
    rpbins = xcf.r_par_min + (
        xcf.r_par_max - xcf.r_par_min
    ) / xcf.num_bins_r_par * np.arange(xcf.num_bins_r_par + 1)
    ...
    rtbins = (
        xcf.r_trans_max / xcf.num_bins_r_trans * np.arange(xcf.num_bins_r_trans + 1)
    )
    ...
    for k in range(num_bins_total):
        dmat[k, pp] = (
            rt_1d_dmat[k % xcf.num_bins_r_trans, pp % xcf.num_bins_r_trans]
            * rp_1d_dmat[k // xcf.num_bins_r_trans, pp // xcf.num_bins_r_trans]
        )
    ...
    sum_out_weight_z, _ = np.histogram(
        output_rp,
        bins=rpbins,
        weights=weights * (((input_zf[:, None] + z_qso[None, :]) / 2.0).ravel()),
    )
...
def main(cmdargs=None):
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Computes metal matrices for the cross-correlation "
    ...
    z_qso = catalog["Z"]
    weight_qso = ((1.0 + z_qso) / (1.0 + args.z_ref)) ** (args.z_evol_obj - 1.0)
