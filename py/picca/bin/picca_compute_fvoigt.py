...
def profile_wave_to_comov_dist(wave, profile_wave, cosmo, differential=False):
    ...
    if differential:
        ...
        profile_comov_dist = (
            cosmo.get_hubble(z) * constants.ABSORBER_IGM["LYA"] / speed_of_light
        )

    profile_comov_dist = np.interp(
        lin_spaced_comov_dist, comov_dist, profile_comov_dist
    )
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "File with probability of finding DLAs in a forest "
            "as a function of observed wavelength"
        ),
    )

    parser.add_argument("--z-dla", type=float, default=None, help=("Mean DLA redshift"))
    ...
    for index, NHI in enumerate(dN_NHI):
        ...
        comov_dist, profile_comov_dist = profile_wave_to_comov_dist(
            wave, profile_wave, fidcosmo
        )
        k, fourier_profile = fft_profile(
            profile_comov_dist, np.abs(comov_dist[1] - comov_dist[0])
        )

        integrand[index] = fourier_profile * mean_density * cddf_NHI[index]

    Fvoigt = np.zeros(k.size)
    for index in range(k.size):
        Fvoigt[index] = np.trapz(integrand[:, index], dN_NHI)
    ...
    np.savetxt(args.output, np.c_[k, Fvoigt], header="k[hMpc^-1] FVoigt")
