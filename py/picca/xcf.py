    ...
    for p in pix:
        for d in dels[p]:
            ...
            for q in sp.array(d.qneighs)[w]:
                ...
                zwe = (1+zd_abs[:,None])**(alpha_abs[abs_igm]-1)/(1+zref)**(alpha_abs[abs_igm]-1)
                ...
                wAB = wA&wBma
                c = sp.bincount(bBma[wAB]+npm*ntm*bA[wAB],weights=wdq[wAB]*zwe)
                dm[:len(c)]+=c

                c = sp.bincount(bBma[wAB],weights=rp_abs[wAB]*wdq[wAB]*zwe)
                rpeff[:len(c)]+=c
                c = sp.bincount(bBma[wAB],weights=rt_abs[wAB]*wdq[wAB]*zwe)
                rteff[:len(c)]+=c
                c = sp.bincount(bBma[wAB],weights=(zd_abs+zq)[wAB]/2*wdq[wAB]*zwe)
                zeff[:len(c)]+=c
                c = sp.bincount(bBma[wAB],weights=wdq[wAB]*zwe)
                weff[:len(c)]+=c
