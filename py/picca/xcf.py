...
def xcf(pix):
    ...
    hist = sp.zeros(npb*ntb)
    whist = sp.zeros(npb*ntb)
        ...
        for d in dels[ipix]:
            ...
            if (d.qneighs.size != 0):
                ...
                if ang_correlation:
                    ...
                    cw,cd,crp,crt,cz,cnb,chist,cwhist = fast_xcf(d.z,10.**d.ll,10.**d.ll,d.we,d.de,zqso,l_qso,l_qso,we_qso,ang)
                else:
                    rc_qso = [q.r_comov for q in d.qneighs]
                    rdm_qso = [q.rdm_comov for q in d.qneighs]
                    cw,cd,crp,crt,cz,cnb,chist,cwhist = fast_xcf(d.z,d.r_comov,d.rdm_comov,d.we,d.de,zqso,rc_qso,rdm_qso,we_qso,ang)
