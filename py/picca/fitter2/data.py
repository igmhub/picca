...
class data:
    def __init__(self,dic_init):
        ...
        bin_center_rp = npy.zeros(rp.size)
        ...
        bin_center_rt = npy.zeros(rt.size)
        ...
        self.mask = mask
        self.da = da
        self.da_cut = npy.zeros(mask.sum())
        self.da_cut[:] = da[mask]
        self.co = co
        ico = co[:,mask]
        ico = ico[mask,:]
        ...
        self.musquare = npy.zeros(self.rsquare.size)
        w = self.rsquare>0.
