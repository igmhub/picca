...
class data:
    ...
    def __init__(self, dic_init):
        ...
        if "hcd_model" in dic_init:
            self.pk = pk.pk(
                getattr(pk, dic_init["model"]["model-pk"]),
        ...
        if dic_init["model"]["growth function"] in ["growth_factor_de"]:
            self.growth_function = partial(
                getattr(xi, dic_init["model"]["growth function"]),
                zref=zref,
                Om=Om,
                OL=OL,
            )
        ...
        if "relativistic correction" in dic_init["model"]:
            self.xi_rel_model = partial(
                getattr(xi, dic_init["model"]["relativistic correction"]),
                name=self.name,
            )
    ...
    def chi2(self, k, pk_lin, pksb_lin, full_shape, pars):
        ...
        pars["sigmaNL_par"] = 0.0
        pars["sigmaNL_per"] = 0.0
