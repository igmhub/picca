    ...
    def __init__(self, dic_init):
        ...
        if 'hcd_model' in dic_init:
            self.pk = pk.pk(getattr(pk, dic_init['model']['model-pk']),
                            dic_init['hcd_model']['name_hcd_model'])
        else:
            self.pk = pk.pk(getattr(pk, dic_init['model']['model-pk']))
        ...
        if 'small scale nl' in dic_init['model']:
            self.pk *= partial(getattr(pk, dic_init['model']['small scale nl']),
                               pk_fid=dic_init['model']['pk'] *
                               ((1 + zref) / (1. + zeff))**2)
        ...
        self.z_evol = {}
        self.z_evol[self.tracer1['name']] = partial(getattr(
            xi, dic_init['model']['z evol {}'.format(self.tracer1['name'])]),
                                                    zref=zeff)
        self.z_evol[self.tracer2['name']] = partial(getattr(
            xi, dic_init['model']['z evol {}'.format(self.tracer2['name'])]),
                                                    zref=zeff)
        ...
        if 'relativistic correction' in dic_init['model']:
            self.xi_rel_model = partial(getattr(
                xi, dic_init['model']['relativistic correction']),
                                        name=self.name)
        ...
        if 'broadband' in dic_init:
            for ibb, dic_bb in enumerate([
                ...
                bb = partial(getattr(xi, dic_bb['func']),
                             ...
                             deg_r_max=deg_r_max,
                             ddeg_r=ddeg_r,
                             deg_mu_min=deg_mu_min,
                             deg_mu_max=deg_mu_max,
                             ddeg_mu=ddeg_mu,
                             rp_rt=dic_bb['rp_rt'] == 'rp,rt',
            ...
            size_bb = len(self.bb['pre-add']) + len(self.bb['pos-add']) + len(
                self.bb['pre-mul']) + len(self.bb['pos-mul'])
            for ibb, dic_bb in enumerate([
                    el for el in dic_init['broadband']
                    if el['func'] == 'broadband_sky'
            ]):
        ...
        if 'metals' in dic_init:
            ...
            assert 'in tracer1' in dic_init[
                'metals'] or 'in tracer2' in dic_init['metals']
            ...
            if 'in tracer1' in dic_init['metals'] and 'in tracer2' in dic_init[
                    'metals']:
