...
def cache_xi_drp(function):
    ...
    def wrapper(*args, **kwargs):
        ...
        if not recalc:
            ...
            cache[pair] = [[bias1, bias2, beta1, beta2, ap, at, drp],
                           xi_drp(*args, **kwargs)]
            ret = cache[pair][1]
...
def xi_qso_radiation(r, mu, tracer1, tracer2, **pars):
    ...
    xi_rad *= np.exp(-r_shift * ((1. + mu_shift) / pars["qso_rad_lifetime"] +
                                 1. / pars["qso_rad_decrease"]))
...
def broadband_sky(r, mu, name=None, bin_size_rp=None, *pars, **kwargs):
    ...
    rp = r * mu
    rt = r * np.sqrt(1 - mu**2)
    cor = kwargs[name + '-scale-sky'] / (kwargs[name + '-sigma-sky'] * np.sqrt(
        2. * np.pi)) * np.exp(-0.5 * (rt / kwargs[name + '-sigma-sky'])**2)
