...
def extract_h5file(fname):
    ...
    err_pars.update({el: 0.0 for el in fixed_p})
...
def apply_mask(data):
    for k in data.dm_dp.keys():
        data.dm_dp[k][~data.mask] = 0.0
    data.ico = linalg.inv(data.co)
    data.ico[:, ~data.mask] = 0.0
    data.ico[~data.mask, :] = 0.0
    data.z[~data.mask] = 0.0
...
def plot_xi(xi, data, title=" "):
    plt.figure()
    plt.imshow(
        xi.reshape((data["np"], data["nt"])),
        origin=0,
        interpolation="nearest",
        extent=[data["rt_min"], data["rt_max"], data["rp_min"], data["rp_max"]],
        cmap="seismic",
        vmin=-max(abs(xi.min()), abs(xi.max())),
