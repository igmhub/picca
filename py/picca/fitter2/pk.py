...
def pk_gauss_exp_smoothing(k, pk_lin, tracer1, tracer2, **kwargs):
    ...
    return np.exp(-(kp**2 * sp2 + kt**2 * st2) /
                  2.) * np.exp(-(np.absolute(kp) * ep2 + np.absolute(kt) * et2))
...
def pk_velo_lorentz(k, pk_lin, tracer1, tracer2, **kwargs):
    ...
    if tracer1['type'] == 'discrete':
        smooth *= 1. / np.sqrt(
            1. + (kp * kwargs['sigma_velo_lorentz_' + tracer1['name']])**2)
    if tracer2['type'] == 'discrete':
        smooth *= 1. / np.sqrt(
            1. + (kp * kwargs['sigma_velo_lorentz_' + tracer2['name']])**2)
    return smooth
