...
def compute_Pk_noise(dll,iv,diff,ll,run_noise):
    ...
    if (run_noise) :
        for iexp in range(nb_noise_exp): #iexp unused, but needed
            delta_exp= sp.zeros(nb_pixels)
            delta_exp[w] = sp.random.normal(0.,err[w])
            k_exp,Pk_exp = compute_Pk_raw(dll,delta_exp,ll) #k_exp unused, but needed
            Pk += Pk_exp

        Pk /= float(nb_noise_exp)

    k_diff,Pk_diff = compute_Pk_raw(dll,diff,ll) #k_diff unused, but needed
