...
def cf(pix):
    ...
    for ipix in pix:
        for i,d1 in enumerate(data1[ipix]):
            sys.stderr.write("\rcomputing xi: {}%".format(round(counter.value*100./(ndata1),2)))
            with lock:
                counter.value += 1
            for d2 in d1.neighs:
                ang = d1^d2
...
def dmat(pix):
    ...
    for p in pix:
        for d1 in data1[p]:
            sys.stderr.write("\rcomputing xi: {}%".format(round(counter.value*100./ndata1,3)))
            with lock:
                counter.value += 1
            r1 = d1.r_comov
            w1 = d1.we
    ...
    for p in pix:
        for d1 in data1[p]:
            ...
            for d2 in sp.array(d1.neighs)[w]:
                ...
                r2 = d2.r_comov
                w2 = d2.we
                fill_dmat_order0_1(l1,r1,r2,w1,w2,ang,wdm,dm,same_half_plate)

    return wdm,dm.reshape(np*nt,np*nt),npairs,npairs_used
    
@jit
def fill_dmat_order0_1(l1,r1,r2,w1,w2,ang,wdm,dm,same_half_plate):
    rp = abs(r1[:,None]-r2)*sp.cos(ang/2)
    rt = (r1[:,None]+r2)*sp.sin(ang/2)
    bp = (rp/rp_max*np).astype(int)
    bt = (rt/rt_max*nt).astype(int)
    ...
    for k,ba in enumerate(bins):
        ...
        j = (ij[k]-i)/n1
        for bb in ubb:
            dm[ba+np*nt*bb] += we[k]*(eta5[bb]+eta7[bb]*dl1[i])\
             - we[k]*(eta1[i+n1*bb]+eta2[j+n2*bb]+eta4[j+n2*bb]*dl1[i])

def dmat_order0_2(pix):
    ...
    for p in pix:
        for d1 in data1[p]:
            ...
            for d2 in sp.array(d1.neighs)[w]:
                ...
                r2 = d2.r_comov
                w2 = d2.we
                fill_dmat_order0_2(r1,r2,w1,w2,ang,wdm,dm,same_half_plate)

    return wdm,dm.reshape(np*nt,np*nt),npairs,npairs_used
    
@jit
def fill_dmat_order0_2(r1,r2,w1,w2,ang,wdm,dm,same_half_plate):
    rp = abs(r1[:,None]-r2)*sp.cos(ang/2)
    rt = (r1[:,None]+r2)*sp.sin(ang/2)
    bp = (rp/rp_max*np).astype(int)
    bt = (rt/rt_max*nt).astype(int)
    ...
    for k,ba in enumerate(bins):
        ...
        j = (ij[k]-i)/n1
        for bb in ubb:
            dm[ba+np*nt*bb] += we[k]*eta5[bb]\
             - we[k]*(eta1[i+n1*bb]+eta2[j+n2*bb])
