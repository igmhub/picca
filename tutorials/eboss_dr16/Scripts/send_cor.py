...
metList = {}
metList["LYA"] = [
    "CIV(eff)", "SiII(1260)", "SiIII(1207)", "SiII(1193)", "SiII(1190)"
]
metList["LYB"] = [
    "CIV(eff)", "SiII(1260)", "SiIII(1207)", "SiII(1193)", "SiII(1190)"
]
...
def send_xcf(zmin, zmax, do_corr, do_dist, do_met, f="LYA", l="LYA"):
    if (zmin == 0.0) and (zmax == 10.0):
        zmin = int(zmin)
        zmax = int(zmax)
        in_dir = path_deltas + "/Delta_{}/Delta/".format(f)
    else:
        if False:
            in_dir = path_deltas + "/Delta_{}_z_{}_{}/Delta/".format(
                f, zmin, zmax)
        else:
    ...
    cmd += " --in-dir " + in_dir
    cmd += " --drq " + path_drq
    cmd += " --out {}/Correlations/xcf_z_{}_{}.fits.gz".format(
        path_here, zmin, zmax)
    cmd += " --z-evol-obj 1.44 "
    ...
    cmd += " --in-dir " + in_dir
    cmd += " --drq " + path_drq
    cmd += " --out {}/Correlations/xdmat_z_{}_{}.fits.gz".format(
        path_here, zmin, zmax)
    cmd += " --z-evol-obj 1.44 "
    cmd += " --fid-Om 0.314569514863487 --fid-Or 7.97505418919554e-5"
    cmd += " --nside 16"
    ...
    cmd = "picca_export.py"
    cmd += " --data {}/Correlations/xcf_z_{}_{}.fits.gz".format(
        path_here, zmin, zmax)
    cmd += " --dmat {}/Correlations/xdmat_z_{}_{}.fits.gz".format(
        path_here, zmin, zmax)
    cmd += " --out {}/Correlations/xcf_z_{}_{}-exp.fits.gz".format(
        path_here, zmin, zmax)
    ...
    cmd += " --in-dir " + in_dir
    cmd += " --drq " + path_drq
    cmd += " --out {}/Correlations/metal_xdmat_z_{}_{}.fits.gz".format(
        path_here, zmin, zmax)
    cmd += " --z-evol-obj 1.44 "
    cmd += " --fid-Om 0.314569514863487 --fid-Or 7.97505418919554e-5"
    cmd += " --nside 16"
    ...
    if (f != "LYA") or (l != "LYA"):
        cmd = cmd.replace("metal_xdmat_",
                          "metal_xdmat_{}_in_{}_".format(strl, f))
...
def send_cf(zmin, zmax, do_corr, do_dist, do_met, f="LYA", l="LYA"):
    ...
    cmd += " --out {}/Correlations/cf_z_{}_{}.fits.gz".format(
        path_here, zmin, zmax)
    ...
    cmd = "picca_dmat.py"
    cmd += " --in-dir {}/Delta_{}/Delta/".format(path_deltas, f)
    cmd += " --out {}/Correlations/dmat_z_{}_{}.fits.gz".format(
        path_here, zmin, zmax)
    # cmd += ' --remove-same-half-plate-close-pairs'
    cmd += " --z-cut-min {} --z-cut-max {}".format(zmin, zmax)
    cmd += " --fid-Om 0.314569514863487 --fid-Or 7.97505418919554e-5"
    ...
    ###
    cmd = "picca_export.py"
    cmd += " --data {}/Correlations/cf_z_{}_{}.fits.gz".format(
        path_here, zmin, zmax)
    cmd += " --dmat {}/Correlations/dmat_z_{}_{}.fits.gz".format(
        path_here, zmin, zmax)
    cmd += " --out {}/Correlations/cf_z_{}_{}-exp.fits.gz".format(
        path_here, zmin, zmax)
    ...
    cmd += " --in-dir {}/Delta_{}/Delta/".format(path_deltas, f)
    cmd += " --out {}/Correlations/metal_dmat_z_{}_{}.fits.gz".format(
        path_here, zmin, zmax)
    cmd += " --z-cut-min {} --z-cut-max {}".format(zmin, zmax)
    # cmd += ' --remove-same-half-plate-close-pairs'
    cmd += " --fid-Om 0.314569514863487 --fid-Or 7.97505418919554e-5"
    ...
    for m in metList[f]:
        cmd += m + " "
    if l != "LYA":
        cmd += " --lambda-abs " + l.replace("dmat_", "dmat_{}_in_{}_".format(
            strl, f))
    if (f != "LYA") or (l != "LYA"):
...
def send_cf_cross(zmin,
    ...
    cmd += " --out {}/Correlations/cf_{}_in_{}_{}_in_{}_z_{}_{}.fits.gz".format(
        path_here, strl1, f1, strl2, f2, zmin, zmax)
    cmd += " --z-cut-min {} --z-cut-max {}".format(zmin, zmax)
    cmd += " --fid-Om 0.314569514863487 --fid-Or 7.97505418919554e-5"
    cmd += " --nside 16"
    print("")
    ...
    cmd += " --in-dir {}/Delta_{}/Delta/".format(path_deltas, f1)
    cmd += " --in-dir2 {}/Delta_{}/Delta/".format(path_deltas, f2)
    cmd += " --out {}/Correlations/dmat_{}_in_{}_{}_in_{}_z_{}_{}.fits.gz".format(
        path_here, strl1, f1, strl2, f2, zmin, zmax)
    cmd += " --z-cut-min {} --z-cut-max {}".format(zmin, zmax)
    cmd += " --fid-Om 0.314569514863487 --fid-Or 7.97505418919554e-5"
    cmd += " --nside 16"
    ...
    cmd = "picca_export.py"
    cmd += " --data {}/Correlations/cf_{}_in_{}_{}_in_{}_z_{}_{}.fits.gz".format(
        path_here, strl1, f1, strl2, f2, zmin, zmax)
    cmd += " --dmat {}/Correlations/dmat_{}_in_{}_{}_in_{}_z_{}_{}.fits.gz".format(
        path_here, strl1, f1, strl2, f2, zmin, zmax)
    cmd += " --out {}/Correlations/cf_{}_in_{}_{}_in_{}_z_{}_{}-exp.fits.gz".format(
        path_here, strl1, f1, strl2, f2, zmin, zmax)
    print("")
    print(cmd)
    if do_dist:
    ...
    cmd += " --in-dir {}/Delta_{}/Delta/".format(path_deltas, f1)
    cmd += " --in-dir2 {}/Delta_{}/Delta/".format(path_deltas, f2)
    cmd += " --out {}/Correlations/metal_dmat_{}_in_{}_{}_in_{}_z_{}_{}.fits.gz".format(
        path_here, strl1, f1, strl2, f2, zmin, zmax)
    cmd += " --z-cut-min {} --z-cut-max {}".format(zmin, zmax)
    cmd += " --fid-Om 0.314569514863487 --fid-Or 7.97505418919554e-5"
    cmd += " --nside 16"
