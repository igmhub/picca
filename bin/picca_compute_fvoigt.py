...
def lambda_to_r(lamb, profile_lambda, fidcosmo):
    ...
    r = fidcosmo.get_r_comov(z)
    rr = np.linspace(r[0], r[-1], r.size)
    profile_lambda = profile_lambda * fidcosmo.get_hubble(
        z) * constants.ABSORBER_IGM["LYA"] / 3e5
...
def main(cmdargs):
    ...
    if args.debug:
        userprint("compute F(k)=int dF/dNHI * dNHI")
    ...
    if args.debug:
        userprint("save in {}".format(args.output))
