...
def get_delta_from_forest(
    ...
    delta = forest.flux / mean_expected_flux_frac - 1.0
    var_pipe = 1.0 / forest.ivar / mean_expected_flux_frac**2
...
def main(cmdargs):
    ...
    if args.zqso_min is None:
        args.zqso_min = max(0.0, args.lambda_min / args.lambda_rest_max - 1.0)
        userprint("zqso_min = {}".format(args.zqso_min))
    if args.zqso_max is None:
        args.zqso_max = max(0.0, args.lambda_max / args.lambda_rest_min - 1.0)
    ...
    # Read data
    (data, num_data, nside, healpy_pix_ordering, blinding) = io.read_data(
        os.path.expandvars(args.in_dir),
        args.drq,
        args.mode,
        z_min=args.zqso_min,
        z_max=args.zqso_max,
    ...
    for iteration in range(num_iterations):
        ...
        if iteration < num_iterations - 1:
            ...
            (
                ...
                mean_cont,
                mean_cont_weight,
            ) = prep_del.compute_mean_cont(data)
        ...
        get_stack_delta = interp1d(
            stack_log_lambda[stack_weight > 0.0],
            stack_delta[stack_weight > 0.0],
            kind="nearest",
            fill_value="extrapolate",
        )
        get_stack_delta_weights = interp1d(
            stack_log_lambda[stack_weight > 0.0],
            stack_weight[stack_weight > 0.0],
            kind="nearest",
            fill_value=0.0,
            bounds_error=False,
        )
    ...
    for healpix in sorted(deltas.keys()):
        if args.delta_format == "Pk1D_ascii":
            ...
            results = fitsio.FITS(
                args.out_dir + "/delta-{}".format(healpix) + ".fits.gz",
                "rw",
                clobber=True,
            )
            for delta in deltas[healpix]:
