...
if __name__ == '__main__':
    ...
    try:
        ...
        multiplier = 10**6
        eval_cut = 0.
        print("Setting negative evals to {}*max_eval to correct".format(multiplier))
        evals,evec_cols=scipy.linalg.eig(co)
        max_eval = max(evals)
        #method from http://www.quarchome.org/correlationmatrix.pdf
        B=sp.zeros(co.shape)
        print('largest eval is:',max_eval)
        for i,eval in enumerate(evals):
            if eval<eval_cut:
                print('{}/{}'.format(i,len(evals)),eval)
                evals[i] = multiplier * max_eval
        diag = sp.diag(evals)
        print('New min eval is:',min(evals))
        co = sp.matmul(evec_cols,sp.matmul(diag,sp.linalg.inv(evec_cols)))
        try:
            scipy.linalg.cholesky(co)
            print('Matrix is now positive definite!')
        except:
            print("Warning: Matrix is still not positive definite")
