...
def cont_fit_dlas(data):
    for i,d in enumerate(data):
        print i,len(data)
        d.cont_fit(fit_dlas=True)
    return data
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ndla',type = int,default=2,required=False,
            help='number of dlas searched')
    ...
    pool.close()
    pool = Pool(processes=args.nproc)
    for it in range(args.ndla):
        data_fit_cont = pool.map(cont_fit_dlas, data.values())
        for i, p in enumerate(data):
            data[p] = data_fit_cont[i]
    pool.close()
    ...
    f.write("## thing_id plate mjd fiber chi2_no_dla ")
    for i in range(args.ndla):
        f.write("chi2_dla_{} zabs_{} nhi_{} ".format(i,i,i))
    f.write("\n")
    for p in data:
        for d in data[p]:
            f.write("{} {} {} {} {} ".format(d.thid,d.plate,d.mjd,d.fid,d.chi2_no_dla))
            for i in range(args.ndla):
                f.write("{} {} {} ".format(d.chi2s[i],d.zabs[i],d.nhi[i]))
            f.write("\n")
