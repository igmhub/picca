...
if __name__ == '__main__':
    ...
    if not args.remove_shuffled_correlation is None:
        """
        #ORIGINAL
        #Remove the average correlation in the randoms in each HP pixel from the data.
        ...
        #NEW
        #Remove the average correlation in the randoms in each bin from the data.
        th = fitsio.FITS(args.remove_shuffled_correlation)
        da_s = th['COR']['DA'][:]
        we_s = th['COR']['WE'][:]

        """
        da_s = (da_s*we_s).sum(axis=0)
        we_s = we_s.sum(axis=0)
        w = we_s>0.
        da_s[w] /= we_s[w]
        th.close()
        da -= da_s[None,:]
        """
        #Remove the shuffled correlation in each bin in each HEALPix pixel from the non-shuffled
        da -= da_s
