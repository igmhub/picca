...
def main():
    ...
    if co.type_corr not in ['DD', 'RR', 'DR', 'RD', 'xDD', 'xRR', 'xD1R2',
                            'xR1D2']:
    ...
    ### Read objects 1
    objs, z_min = io.read_objects(args.drq,
                                  args.nside,
                                  args.z_min_obj,
                                  args.z_max_obj,
                                  args.z_evol_obj,
    ...
    ### Read objects 2
    if co.x_correlation:
        objs2, z_min2 = io.read_objects(args.drq2,
                                        args.nside,
                                        args.z_min_obj,
                                        args.z_max_obj,
                                        args.z_evol_obj2,
    ...
    header = [
        {'name': 'RPMIN', 'value': co.r_par_min,
         'comment': 'Minimum r-parallel [h^-1 Mpc]'},
        {'name': 'RPMAX', 'value': co.r_par_max,
         'comment': 'Maximum r-parallel [h^-1 Mpc]'},
        {'name': 'RTMAX', 'value': co.r_trans_max,
         'comment': 'Maximum r-transverse [h^-1 Mpc]'},
        {'name': 'NP', 'value': co.num_bins_r_par,
         'comment': 'Number of bins in r-parallel'},
        {'name': 'NT', 'value': co.num_bins_r_trans,
         'comment': 'Number of bins in r-transverse'},
        {'name': 'NSIDE', 'value': co.nside,
         'comment': 'Healpix nside'},
        {'name': 'TYPECORR', 'value': co.type_corr,
         'comment':'Correlation type'},
        {'name': 'NOBJ', 'value': len([obj for healpix in co.objs
                                       for obj in co.objs[healpix]]),
         'comment': 'Number of objects'},
    ]
    if co.x_correlation:
        header += [{'name': 'NOBJ2',
                    'value': len([obj2 for healpix in co.objs2
                                  for obj2 in co.objs2[healpix]]),
