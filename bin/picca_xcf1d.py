...
def main():
    ...
    if not args.no_remove_mean_lambda_obs:
        ...
        Forest.log_lambda_min = (np.log10((z_min + 1.)*lambda_abs) -
                                 Forest.delta_log_lambda/2.)
        Forest.log_lambda_max = (np.log10((z_max + 1.)*lambda_abs) +
                                 Forest.delta_log_lambda/2.)
    ...
    for healpix in xcf.objs:
        for obj in xcf.objs[healpix]:
            obj.log_lambda = np.log10((1. + obj.z_qso)*
                                      constants.ABSORBER_IGM[args.lambda_abs_obj])
    sys.stderr.write("\n")
    ...
    healpix_list = np.array([healpix
                             for healpix in sorted(data)
                             if healpix in xcf.objs])
    ...
    header2 = [
        {'name': 'HLPXSCHM', 'value': 'RING', 'comment': 'Healpix scheme'}
    ]
    results.write(
