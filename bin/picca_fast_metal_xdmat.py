...
def calc_fast_metal_dmat(in_lambda_abs, out_lambda_abs, stack_table,
    ...
    if rebin_factor is not None :
        size=loglam.size
        loglam = loglam[:(size//rebin_factor)*rebin_factor].reshape((size//rebin_factor),rebin_factor).mean(-1)
        weight_forest = weight_forest[:(size//rebin_factor)*rebin_factor].reshape((size//rebin_factor),rebin_factor).mean(-1)
    ...
    sum_out_weight_z,_ = np.histogram(output_rp,bins=rpbins,weights=weights*(((output_zf[:,None]+z_qso[None,:])/2.).ravel()))
...
def main():
    ...
    for index, abs_igm in enumerate(args.abs_igm):
        ...
        dmat, r_par_eff, r_trans_eff, z_eff = calc_fast_metal_dmat(abs_igm,
                                                                   args.lambda_abs,
                                                                   stack_table,
                                                                   z_qso,
                                                                   weight_qso,
