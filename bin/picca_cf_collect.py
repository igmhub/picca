...
def main(cmdargs):
    ...
    parser = argparse.ArgumentParser(
        ...
        description='Compute the auto and cross-correlation of delta fields. See tutorials/cf_hack for a script to run this.')
    ...
    parser.add_argument('--in-dir-node',
                        type=str,
                        default=None,
                        required=True,
                        help='Directory to node files')
    ...
    parser.add_argument('--node-ind',
                        type=int,
                        default=None,
                        required=False,
                        help='Node index when cutting data')

    parser.add_argument('--num-nodes',
                        type=int,
                        default=None,
                        required=False,
                        help='Number of nodes when cutting data')
    ...
    if args.num_nodes is None or args.node_ind is None:
        raise ValueError

    cpu_data = {healpix: [healpix] for healpix in data}
#     hp_numbers = list(data.keys())
#     hp_per_node = len(hp_numbers) // args.num_nodes
#     start = hp_per_node * (args.node_ind - 1)
#     end = hp_per_node * args.node_ind
#     if args.node_ind < args.num_nodes:
#         compute_hp = hp_numbers[start:end]
#     else:
#         compute_hp = hp_numbers[start:]
...
#     cf.lock = Lock()
#     cpu_data = {healpix: [healpix] for healpix in compute_hp}
#     print(sorted(cpu_data.values()))
...
#     userprint(f'picca_cf.py - Time computing correlation function: {(t2-t1)/60:.3f} minutes')
    ...
    # pickle.dump(correlation_function_data, open(args.out, "wb"))

    files = []
    for i in range(1, args.num_nodes + 1):
        files.append(args.in_dir_node + '/cf_node_{}.p'.format(i))

    correlation_function_data = []
    for file in files:
        correlation_function_data += pickle.load(open(file, "rb"))
